...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold text-dark">Modifier le role</h2>
    </div>
        ...
        <div class="card-body p-4">
            <form action="{{ route('admin.roles.update', $role->id) }}" method="POST">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="name" class="form-control bg-light border-0 py-2" value="{{ old('name', $role->name) }}" required>
                    @error('name')
                                ...
                                @foreach($actions as $action)
                                    ...
                                    <td class="text-center">
                                        <div class="form-check d-inline-block">
                                            <input class="form-check-input perm-checkbox" type="checkbox" name="permissions[]" value="{{ $permName }}" {{ $checked }}>
                                        </div>
                                    </td>
                                @endforeach
                            </tr>
                ...
                <div class="d-flex justify-content-start">
                    <button type="submit" class="btn btn-dark px-5 py-2 fw-bold shadow-sm">Modifier</button>
                </div>
            </form>
        </div>
    </div>
